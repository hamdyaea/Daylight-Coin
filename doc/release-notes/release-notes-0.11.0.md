Daylight Core version 0.11.0 is now available from:

  <https://daylight.org/bin/daylight-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer DLT/kB fee to be
...
For example, add the following to `daylight.conf`:
...
related to the blockchain in the daylight system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Daylight Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Daylight Core will 
...
For an example script see [daylight-submittx](https://github.com/laanwj/daylight-submittx).
...
- #5334 `cf87536` libdaylightconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new daylight-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only DaylightAmountField
- #5753 `0f44672` Add daylight logo to about screen
...
- #5809 `46bfbe7` Add daylight-cli man page
...
- #5849 `41259ca` contrib/init/daylightd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Daylight-Qt.app to Daylight-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for daylight-tx)
