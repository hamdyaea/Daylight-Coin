#!/bin/bash
# Copyright (c) 2013 The Daylight Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Daylight database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Daylight datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Daylight datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Daylight 0.7 datadir."
    ;;
  3)
    echo "Detected Daylight pre-0.8 datadir."
    ;;
  4)
    echo "Detected Daylight 0.8 datadir."
    ;;
esac
