...
The RPM spec file provided here is for Daylight-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://daylight.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Daylight git repository](https://github.com/daylight/daylight) but are not part of
...
Currently only `Source10` is used, for the example `daylightdaylight.conf` file.
...
in the daylight git repository. At present only an SVG version of the Daylight
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the daylight
...
needed when 0.12.1 is released, a proper fix is already in the Daylight git
master branch.
...
idea to build Daylight Core against a static version of that library compiled
...
The `daylight.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `daylight-server` package. At this point, most RPM based
...
The `daylight-server` package should have SELinux support. How to properly do
...
Daylight-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/daylight/). He has been
packaging Daylight for Fedora at least since 2012.
...
4. I renamed the `daylight` package that contains the Qt GUI to `daylight-core` as
that appears to be how the general population refers to it, in contrast to
`daylight-xt` or `daylight-classic`. I wanted to make sure the general population
