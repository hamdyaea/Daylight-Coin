...
###   daylightcore.org and daylight.org and compares them.
...
WORKINGDIR="/tmp/daylight_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://daylightcore.org"
HOST2="https://daylight.org"
BASEDIR="/bin/"
VERSIONPREFIX="daylight-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'daylight-',
...
if [ $? -ne 0 ]; then
   echo "daylight.org failed to provide signature file, but daylightcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "daylight.org and daylightcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Daylight Core binary release signing key installed?"
   fi
