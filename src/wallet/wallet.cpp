...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CDaylightAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/daylight/daylight/pull/10340, but that change has been
    // postponed until after https://github.com/daylight/daylight/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-daylight-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CDaylightAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CDaylightAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CDaylightAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CDaylightAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CDaylightAddress(address).ToString());
}
 ...
 * https://daylighttalk.org/?topic=54527, or
 * https://github.com/daylight/daylight/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CDaylightAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CDaylightAddress(dest).ToString(), key);
}
