...
double GetDifficultyDaylight(const CBlockIndex* blockindex)
{
...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    if (blockindex->nHeight >= Params().GetConsensus().DLTHeight)
    {
        return GetDifficultyINTERNAL(blockindex);
    }
    else
    {
        return GetDifficultyDaylight(blockindex);
    }
}
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    int ser_flags = (blockindex->nHeight < consensusParams.DLTHeight) ? SERIALIZE_BLOCK_LEGACY : 0;
    result.push_back(
        Pair("strippedsize",
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of daylight addresses\n"
