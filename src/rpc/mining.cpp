...
{
    static const int nInnerLoopDaylightMask = 0x1FFFF;
    static const int nInnerLoopDaylightCount = 0x10000;
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (pblock->nHeight < (uint32_t)params.GetConsensus().DLTHeight) {
            // Solve sha256d.
            nInnerLoopMask = nInnerLoopDaylightMask;
            nInnerLoopCount = nInnerLoopDaylightCount;
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated daylight to.\n"
...
// NOTE: Unlike wallet RPC (which use DLT values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "    https://github.com/daylight/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/daylight/bips/blob/master/bip-0023.mediawiki\n"
            "    https://github.com/daylight/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
            "    https://github.com/daylight/bips/blob/master/bip-0145.mediawiki\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Daylight is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Daylight is downloading blocks...");
...
UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3) {
        throw std::runtime_error(
            ...
            "See https://en.daylight.it/wiki/BIP_0022 for full specification.\n"
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric, optional) estimate fee-per-kilobyte (in DLT)\n"
