// Copyright (c) 2017 The Daylight Gold developers
...
#include "test/dlt_cltv_multisig_data.h"
...
BOOST_FIXTURE_TEST_SUITE(dlt_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(zcash_header_compatible)
{
    ...
    dlt_header.nHeight = 10000000;
    // `CBlockHeader.nReserved[5:7]` should be placed in the last 8 bytes of `ZcashBlockHeader.hashReserved`.
    dlt_header.nReserved[5] = 1234;
    dlt_header.nReserved[6] = 0;
    dlt_header.nSolution = std::vector<unsigned char> {
        0x11, 0x22, 0x33
    };
    ...
    BOOST_CHECK_EQUAL(dlt_header.nHeight, (uint32_t)zcash_header.hashReserved.GetUint64(0));
    BOOST_CHECK_EQUAL(dlt_header.nReserved[5], (uint32_t)zcash_header.hashReserved.GetUint64(3));
    BOOST_CHECK_EQUAL(dlt_header.nSolution[2], zcash_header.nSolution[2]);
}
...
std::string ConvertToNewAddress(const std::string& old_address)
{
    ...
    CDaylightAddress new_addr(addr.Get(fakeLegacyChainParam));
    return new_addr.ToString();
}

std::string ConvertToOldAddress(const std::string& new_address)
{
    CDaylightAddress addr(new_address);
    BOOST_CHECK(addr.IsValid());
    CDaylightAddress old_addr(addr.Get(), fakeLegacyChainParam);
