...
const char * const BITCOIN_CONF_FILENAME = "daylightdaylight.conf";
const char * const BITCOIN_LEGACY_CONF_FILENAME = "daylight.conf";
const char * const BITCOIN_PID_FILENAME = "bdaylightd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DaylightGold
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DaylightGold
    // Mac: ~/Library/Application Support/DaylightGold
    // Unix: ~/.daylightdaylight
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DaylightGold";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DaylightGold";
#else
    // Unix
    return pathRet / ".daylightdaylight";
#endif
#endif
}
    ...
    if (streamConfig.good()) {
        {
            ...
            for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
                // Don't overwrite existing settings so command line settings override daylightdaylight.conf
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Daylight Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Daylight Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Daylight Core developers";
    }
