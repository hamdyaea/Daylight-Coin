...
#if defined(NDEBUG)
# error "Daylight cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Daylight Gold Signed Message:\n";
...
bool static IsDLTHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.DLTHeight;
}

bool IsDLTHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsDLTHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsDLTHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsDLTHardForkEnabled(chainActive.Tip(), params);
}
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s DLT additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.DLTHeight;
...
void ThreadScriptCheck() {
    RenameThread("daylight-scriptch");
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsDLTHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.DLTHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after DLT fork.
    if (nHeight >= consensusParams.DLTHeight && block.nHeight != (uint32_t)nHeight)
    ...
    // Check timestamp
    if (block.GetBlockTime() > nAdjustedTime + std::min(consensusParams.DLTMaxFutureBlockTime,
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.DLTHeight &&
        nHeight < consensusParams.DLTHeight + consensusParams.DLTPremineWindow &&
        consensusParams.DLTPremineEnforceWhitelist)
    {
